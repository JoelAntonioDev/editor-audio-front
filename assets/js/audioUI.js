class AudioUI {
    constructor(audioElement, currentTimeElement, durationElement, timeListContainer) {
    ...
    updateTimeDisplay() {
        this.audio.addEventListener('timeupdate', () => {
            ...
            this.durationElement.textContent = `${durationMinutes}:${durationSeconds.toString().padStart(2, '0')}`;
        });
    }
    ...
    generateTimeList() {
        this.audio.addEventListener('loadedmetadata', () => {
            ...
            for (let i = 0; i < totalSeconds; i += interval) {
                ...
                li.textContent = `${Math.floor(i / 60)}:${(i % 60).toString().padStart(2, '0')}`;
                list.appendChild(li);
            }
    ...
    createSoundTimeController(audioURL, fileName, audioBuffer) {
        const audioViewer = document.querySelector(".audio-viewer");

        // Cria um novo contêiner para o áudio
        const audioContainer = document.createElement('div');
        audioContainer.classList.add('audio-container');
        audioContainer.style.cssText = 'display: flex; flex-direction: column; margin-bottom: 20px; width: 100%';

        const titleDiv = document.createElement('div');
        titleDiv.style.cssText = 'display: flex; flex-direction: row; gap: 25px; align-items: center; color: black';
        ...
        mixer.style.cssText = 'height: 55px; width: 100%; position: relative; background-color: #dbdada;';

        const bar = document.createElement('div');
        bar.classList.add('bar');
        bar.style.cssText = 'position: absolute; top: 0; bottom: 0; width: 4px; background-color: white; left: 0; cursor: pointer;';
        ...
        timeReproducer.style.cssText = 'display: flex; flex-direction: row; width: 100%; height: 20px; background-color: aliceblue; display: none';
        ...
        audioElement.addEventListener('loadedmetadata', () => {
            ...
            let li = document.createElement('li');
            li.innerText = `${durationMinutes}:${durationSeconds < 10 ? '0' : ''}${durationSeconds}`;
            lista.appendChild(li);
        ...
        audioTitle.style.width = '20ch'; // Define a largura máxima do texto
        audioTitle.style.whiteSpace = 'nowrap'; // Impede quebra de linha
        audioTitle.style.overflow = 'hidden'; // Oculta texto excedente
        audioTitle.style.textOverflow = 'ellipsis'; // Adiciona "..." ao final do texto
                ...
                allAudioElements.forEach(audio => {
                    if (!audio.paused) {
                        ...
                        const pausedButton = document.querySelector(
                            `button[id="play-button-${Array.from(allAudioElements).indexOf(audio)}"]`
                        );
                        if (pausedButton) {
        ...
        audioControlsDivRightBottom.appendChild(volumeLabel);
        audioControlsDivRightBottom.appendChild(volumeControl);

        //audioContainer.appendChild(audioControlsDiv);

        audioViewer.appendChild(audioContainer);

        //posicao dos controllers
        this.posicionarTracks(audioContainer, this.audioIdCounter, audioControlsDiv);
    ...
    setupAudioControls(audio, bar, mixer, timeReproducer) {
        ...
        audio.addEventListener('timeupdate', () => {
            if (!isMoving) {
                const progress = (audio.currentTime / audio.duration) * mixer.offsetWidth;
    ...
    // Pegar a posição dos buttons para posicionar a tracks e as mixes de cada track
    posicionarTracks(audioContainer, audioCounter, audioControlsDiv) {
        ...
        if (this.audioControllers2) {
            ...
            if (audioCounter === 1) {
                ...
                const previousAudio = document.querySelectorAll('.audio-container')[audioCounter - 2]; // Pega o áudio anterior
            ...
            if (audioContainer) {
                ...
                console.log(`Novo top do .audio-container: ${audioContainer.style.top}`);
                console.log(`Novo height do .audio-controlsDiv: ${audioControlsDiv.style.height}`);
            } else {
